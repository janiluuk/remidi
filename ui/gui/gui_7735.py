...
if SCNTYPE == 1:
    device = get_device()
#st7735(serial, rotate=2, bgr=1) #sh1106
...
def switch_menu(argument):
    ...
    0: "_  Settings",
        ...
        40: "_NETWORK",
        41: "_",
        42: "_Inject Bunnyshell(ADMIN)",
        43: "_Inject Bunnyshell (USER)",
        44: "_Apple Airplay Exploit",
        45: "_",
        46: "_",
        47: "_",
        48: "_"
...
def OLEDContrast(contrast):
    ...
    return(contrast)
def splash():
    img_path = os.path.abspath(os.path.join(os.path.dirname(__file__), 'images', 'remidi_128.bmp'))
...
def runhid():
    ...
    ""
        ...
        if answer == 1:
            # run as background job remidi_cli hid job command
            cmd = "remidi_cli hid job '" + fichier+"'"
            result=subprocess.check_output(cmd, shell = True )
            return()
        if answer == 3:
            # run hid script directly
            cmd = "remidi_cli hid run '" + fichier+"'"
            result=subprocess.check_output(cmd, shell = True )
            return()
def restart():
    DisplayText(
    ...
    cmd="python3 /home/pi/remidi/ui/gui/runmenu.py &"
...
def ApplyTemplate(template,section):
    ...
    while GPIO.input(KEY_LEFT_PIN):
        ...
        if answer == 2:
            return()
        time.sleep(0.5) #pause
        cmd = "remidi_cli template deploy -" +section + " '"+ template+"'"
        exe = subprocess.check_output(cmd, shell = True )
        return()
def Gamepad():
    if SCNTYPE == 1:
        while GPIO.input(KEY_PRESS_PIN):
            with canvas(device) as draw:
                if GPIO.input(KEY_UP_PIN): # button is released
                        ...
                        draw.polygon([(20, 20), (30, 2), (40, 20)], outline=255, fill=1)  #Up filled
                        exe = subprocess.check_output("remidi_cli hid run -c 'press(\"UP\")'", shell = True )

                if GPIO.input(KEY_LEFT_PIN): # button is released
                        draw.polygon([(0, 30), (18, 21), (18, 41)], outline=255, fill=0)  #left
                else: # button is pressed:
                        draw.polygon([(0, 30), (18, 21), (18, 41)], outline=255, fill=1)  #left filled
                        exe = subprocess.check_output("remidi_cli hid run -c 'press(\"LEFT\")'", shell = True )

                if GPIO.input(KEY_RIGHT_PIN): # button is released
                        draw.polygon([(60, 30), (42, 21), (42, 41)], outline=255, fill=0) #right
                else: # button is pressed:
                        draw.polygon([(60, 30), (42, 21), (42, 41)], outline=255, fill=1) #right filled
                        exe = subprocess.check_output("remidi_cli hid run -c 'press(\"RIGHT\")'", shell = True )

                if GPIO.input(KEY_DOWN_PIN): # button is released
                        draw.polygon([(30, 60), (40, 42), (20, 42)], outline=255, fill=0) #down
                else: # button is pressed:
                        draw.polygon([(30, 60), (40, 42), (20, 42)], outline=255, fill=1) #down filled
                        exe = subprocess.check_output("remidi_cli hid run -c 'press(\"DOWN\")'", shell = True )

                if GPIO.input(KEY1_PIN): # button is released
                        draw.ellipse((70,0,90,20), outline=255, fill=0) #A button
                else: # button is pressed:
                        draw.ellipse((70,0,90,20), outline=255, fill=1) #A button filled
                        exe = subprocess.check_output("remidi_cli hid run -c 'press(\"Q\")'", shell = True )

                if GPIO.input(KEY2_PIN): # button is released
                        draw.ellipse((100,20,120,40), outline=255, fill=0) #B button
                else: # button is pressed:
                        draw.ellipse((100,20,120,40), outline=255, fill=1) #B button filled
                        exe = subprocess.check_output("remidi_cli hid run -c 'press(\"W\")'", shell = True )
                        
                if GPIO.input(KEY3_PIN): # button is released
                        draw.ellipse((70,40,90,60), outline=255, fill=0) #A button
                else: # button is pressed:
                        draw.ellipse((70,40,90,60), outline=255, fill=1) #A button filled
                        exe = subprocess.check_output("remidi_cli hid run -c 'press(\"E\")'", shell = True )
def Mouse():
    bouton1 = 0
    bouton2 = 0
    step = 10
        ...
        while GPIO.input(KEY2_PIN):
            with canvas(device) as draw:
                if GPIO.input(KEY_UP_PIN): # button is released
                        ...
                        draw.polygon([(20, 20), (30, 2), (40, 20)], outline=255, fill=1)  #Up filled
                        exe = subprocess.check_output("remidi_cli hid run -c 'moveStepped(0,-"+str(step)+")'", shell = True )

                if GPIO.input(KEY_LEFT_PIN): # button is released
                        draw.polygon([(0, 30), (18, 21), (18, 41)], outline=255, fill=0)  #left
                else: # button is pressed:
                        draw.polygon([(0, 30), (18, 21), (18, 41)], outline=255, fill=1)  #left filled
                        exe = subprocess.check_output("remidi_cli hid run -c 'moveStepped(-"+str(step)+",0)'", shell = True )
                ...
                if GPIO.input(KEY_PRESS_PIN): # button is released
                        ...
                        draw.rectangle((20, 22,40,40), outline=255, fill=1) #center filled
                        if step == 10:
                            #exe = subprocess.check_output("remidi_cli hid run -c 'button(BT1)'", shell = True )
                            step = 100
                            time.sleep(0.2)
                        else:
                            #exe = subprocess.check_output("remidi_cli hid run -c 'button(BTNONE)'", shell = True )
                            step = 10
                            time.sleep(0.2)
                ...
                if GPIO.input(KEY3_PIN): # button is released
                        ...
                        draw.ellipse((70,40,90,60), outline=255, fill=1) #A button filled
                        if bouton2 == 0:
                            exe = subprocess.check_output("remidi_cli hid run -c 'button(BT2)'", shell = True )
                            bouton2 = 1
                            time.sleep(0.2)
                            exe = subprocess.check_output("remidi_cli hid run -c 'button(BTNONE)'", shell = True )
...
def SetTypingSpeed():
    ...
    while GPIO.input(KEY_LEFT_PIN):         
        ...
        if GPIO.input(KEY1_PIN): # button is released
            ...
            exe = subprocess.check_output("remidi_cli hid run -c 'typingSpeed(100,150)'", shell = True )
            time.sleep(0.5) #pause
            return()
        if GPIO.input(KEY2_PIN): # button is released
            menu = 1
        ...
        if GPIO.input(KEY3_PIN): # button is released
            ...
            exe = subprocess.check_output("remidi_cli hid run -c 'typingSpeed(0,0)'", shell = True )
            time.sleep(0.5) #pause
            return()
    time.sleep(0.5) #pause
def ListWifi ():
    ...
    while GPIO.input(KEY_PRESS_PIN):
        with canvas(device) as draw:
            if GPIO.input(KEY_UP_PIN): # button is released
                    ...
                    draw.polygon([(20, 20), (30, 2), (40, 20)], outline=255, fill=1)  #Up filled
                    shell("remidi_cli trigger send -n \"oled\" -v 1")
            ...
            if GPIO.input(KEY_LEFT_PIN): # button is released
                    ...
                    draw.polygon([(0, 30), (18, 21), (18, 41)], outline=255, fill=1)  #left filled
                    shell("remidi_cli trigger send -n \"oled\" -v 3")
            ...
            if GPIO.input(KEY_RIGHT_PIN): # button is released
                    ...
                    draw.polygon([(60, 30), (42, 21), (42, 41)], outline=255, fill=1) #right filled
                    shell("remidi_cli trigger send -n \"oled\" -v 4")
            ...
            if GPIO.input(KEY_DOWN_PIN): # button is released
                    ...
                    draw.polygon([(30, 60), (40, 42), (20, 42)], outline=255, fill=1) #down filled
                    shell("remidi_cli trigger send -n \"oled\" -v 2")
            ...
            if GPIO.input(KEY1_PIN): # button is released
                    ...
                    draw.ellipse((70,0,90,20), outline=255, fill=1) #A button filled
                    shell("remidi_cli trigger send -n \"oled\" -v 10")
            ...
            if GPIO.input(KEY2_PIN): # button is released
                    ...
                    draw.ellipse((100,20,120,40), outline=255, fill=1) #B button filled
                    shell("remidi_cli trigger send -n \"oled\" -v 20")
            ...
            if GPIO.input(KEY3_PIN): # button is released
                    ...
                    draw.ellipse((70,40,90,60), outline=255, fill=1) #A button filled
                    shell("remidi_cli trigger send -n \"oled\" -v 30")
...
def menu2():
    ...
    shell("remidi_cli hid job 'GetChrome.js'")
    hack = ""
                ...
                if curseur == 1:
                    ...
                    while answer == 0:
                        ...
                        if GPIO.input(KEY3_PIN): # button is released
                            ...
                            answer = 2
                    if answer == 1:
                        shell("remidi_cli hid job 'gui inject revshell.js'")
                if curseur == 3:
                ...
                if curseur == 2:
                    ...
                    while answer == 0:
                        ...
                        if GPIO.input(KEY3_PIN): # button is released
                            ...
                            answer = 2
                    if answer == 1:
                        shell("remidi_cli hid job 'gui inject revshell user.js'")                    
            if page == 0:
            #we are in main menu
                if curseur == 1:
                    # call about
